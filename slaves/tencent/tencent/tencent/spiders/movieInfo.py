...
from makeUrlList import MovieUrlListSpider
from scrapy import log


urlLists = []
print len(urlLists),"is in database now !"

class MovieinfoSpider(scrapy.Spider):
    name = "movieInfo"
    print "spider name is :",name
    allowed_domains = ["qq.com"]
    start_urls = MovieUrlListSpider().MovieUrlList

    def start_requests(self):
        log.msg("will start the first request",level=log.INFO)
        for items in self.start_urls:
            log.msg("load item:{}".format(items),level=log.INFO)
            types = items.keys()[0]
            for url in items.values()[0]:
                print types,url,"###"
                yield self.make_requests_from_url(types,url)

    def make_requests_from_url(self,types,url):
        return scrapy.Request(meta={"types":types},url=url)

    def parse(self, response):
        types = response.meta["types"]
        codes = response.body
        splitPatterns = re.findall(r'<li class="list_item">[\s\S]*?</li>',codes)
